	...
	<van-form @submit="onSubmit" style="margin:3rem;">
		...
		<van-cell-group>
			...
			<van-nav-bar v-bind:title="material" :left-text="i18n.previous[0]" :right-text="i18n.next[0]" @click-left="onClickLeft"
			 @click-right="onClickRight" :color="'#4fc08d'" />
		...
		<van-cell-group  v-show='i18n.destM_Str1[1]==1'>
			<van-field v-model="destM_Str1" :label="i18n.destM_Str1[0]" readonly/>
		</van-cell-group>
		<van-cell-group  v-show='i18n.destM_Str2[1]==1'>
			<van-field v-model="destM_Str2" :label="i18n.destM_Str2[0]" readonly/>
		</van-cell-group>
		<van-cell-group  v-show='i18n.destM_Str3[1]==1'>
			<van-field v-model="destM_Str3" :label="i18n.destM_Str3[0]" readonly/>
		</van-cell-group>
		<van-cell-group  v-show='i18n.destM_Str4[1]==1'>
			<van-field v-model="destM_Str4" :label="i18n.destM_Str4[0]" readonly/>
		</van-cell-group>
		<van-cell-group  v-show='i18n.destM_Str5[1]==1'>
			<van-field v-model="destM_Str5" :label="i18n.destM_Str5[0]" readonly/>
		</van-cell-group>
		<van-cell-group  v-show='i18n.destM_Str6[1]==1'>
			<van-field v-model="destM_Str6" :label="i18n.destM_Str6[0]" readonly/>
		</van-cell-group>
		<van-cell-group  v-show='i18n.destM_Str7[1]==1'>
			<van-field v-model="destM_Str7" :label="i18n.destM_Str7[0]" readonly/>
		</van-cell-group>
		<van-cell-group  v-show='i18n.destM_Str8[1]==1'>
			<van-field v-model="destM_Str8" :label="i18n.destM_Str8[0]" readonly/>
		</van-cell-group>
		<van-cell-group  v-show='i18n.destM_Str9[1]==1'>
			<van-field v-model="destM_Str9" :label="i18n.destM_Str9[0]" readonly/>
		</van-cell-group>
		<van-cell-group  v-show='i18n.destM_Str10[1]==1'>
			<van-field v-model="destM_Str10" :label="i18n.destM_Str10[0]" readonly/>
		</van-cell-group>
		...
		<van-cell-group v-show='i18n.pickingnumber[1]==1' >
			<van-field v-model="planPkgQuantity" :label="i18n.pickingnumber[0]" type="number" 
			 readonly>
				<template #button>
					<p>{{quantity}}</p>
				</template>
		...
		<van-cell-group v-show='i18n.tocarton[1]==1'>
			<van-field v-model="toCarton" :label="i18n.tocarton[0]" />
		</van-cell-group>
	
		<van-cell-group v-show='i18n.backpallet[1]==1'>
			<van-field v-model="backPallet" :label="i18n.backpallet[0]" />
		</van-cell-group>
	
		<van-cell-group v-show='i18n.backcarton[1]==1'>
			<van-field v-model="backCarton" :label="i18n.backcarton[0]" />
		</van-cell-group>
...
<script>
	...
	export default {
		...
		computed: {
			i18n() {
				return this.$t('autoTransferHandle')
			}
		},
	    mounted:function(){
	    		uni.setNavigationBarTitle({
	    		            title:this.$t('autoTransferHandle').title
	    		        });
				this.$nextTick(() => {
		 ...
		 palletNo:function (str) {
		        if (typeof str ==='string'){
		            if (str.trim().length!==0){
		                this.debounce(this.onChange,2000);
		            }else {}
		        }
		    },
		code:function (str){
			    if (str.trim().length!==0){
			        this.debounce(this.onChangeCode,1000);
			}
		}
		},
		methods: {
			onChange() {
				...
				if (this.palletNo) {
					...
					axios.post(this.apiUrl.apiUrl + '/api/app/queryManager/inventoryTransferWmsTask?pallet='+this.palletNo)
						.then(function(response) {
	
							_this.wmsTasks = response.data;
							...
							if (_this.wmsTasks.length > 0) {
								...
								_this.material =`${wmsTask.material.xCode}
								...
								_this.destM_Str1=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str1:'';
								_this.destM_Str2=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str2:'';
								_this.destM_Str3=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str3:'';
								_this.destM_Str4=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str4:'';
								_this.destM_Str5=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str5:'';
								_this.destM_Str6=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str6:'';
								_this.destM_Str7=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str7:'';
								_this.destM_Str8=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str8:'';
								_this.destM_Str9=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str9:'';
								_this.destM_Str10=wmsTask.DestMaterialProperty!=null?wmsTask.DestMaterialProperty.m_Str10:'';
						...
						.catch(function(error) { // 请求失败处理
							...
							Toast({
								message: _this.$t('autoTransferHandle').taskErrorMsg
							});
						});
				}
			},
			...
			onClickLeft() {
				...
				if (this.i > 0) {
					...
					this.material = `${wmsTask.material.xCode}
					${wmsTask.material.xName}
					${wmsTask.material.spec==null?"":wmsTask.material.spec}`;
			...
			onSubmit() {
				...
				this.loading = true;
				var _this = this;
				axios.post(this.apiUrl.apiUrl + '/api/app/WorkDocManager/ConfirmByPallet?pallet='+this.palletNo
						)
					.then(function(response) {
