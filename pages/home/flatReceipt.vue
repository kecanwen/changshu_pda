	...
	export default {
		...
		methods: {
			...
			onSubmit() {
				...
				let _this = this;
				let inventoryStatuCode=this.$t('flatReceipt').inventoryStatusCode[this.inventoryStatus.indexOf(this.inventoryStatu)];
				...
				let input2={
					...
					else if(this.$t('flatReceipt').receiptLocation[1]==1)
					{
						input['LocationId']=this.receiveLocId
					}
				
				
				axios.post(this.apiUrl.apiUrl + '/api/app/autoInboundManager/receive',input2 )
					.then(function(response) {
