	...
	<van-form @submit="onSubmit" style="margin: 3rem;">
		...
		<van-pull-refresh v-model="isLoading" @refresh="onRefresh">
			...
			<van-cell-group  v-show='i18n.palletNo[1]==1'>
				<!--
                :rules="[{ required: true, message: i18n.palletNoMsg}]"
                required
                -->
	...
	export default {
		...
		methods: {
			...
			onSubmit() {
				...
				let _this = this;
				let inventoryStatuCode=this.$t('receipt').inventoryStatusCode[this.inventoryStatus.indexOf(this.inventoryStatu)];
					...
					IsWeipan:this.isWeipan==true?'Y':'N'
						...
						.then(function(response) {
							...
							if (response.data == true) {
								...
								_this.receiptOrderList.find(e => e.id == _this.receiptOrderId).receiptOrderItem.find(e=>e.id==_this.receiptOrderItemId).expectedPkgQuantity = _this.receiptOrderItemList
										.find(e => e.id == _this.receiptOrderItemId).expectedPkgQuantity-_this.receiptOrderItemList
