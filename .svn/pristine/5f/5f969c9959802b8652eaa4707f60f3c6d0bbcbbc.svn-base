<template>
	<van-form @submit="onSubmit" style="margin:3rem;">
		...
		<van-cell-group v-show='i18n.pickingnumber[1]==1'>
			<van-field v-model="planPkgQuantity" :label="i18n.pickingnumber[0]" type="number" readonly
			 >
				<template #button>
					<p>{{quantity}}</p>
				</template>
...
<script>
	...
	export default {
		...
		computed: {
			i18n() {
				return this.$t('autoHandle')
			}
		},
	    mounted:function(){
	    		uni.setNavigationBarTitle({
	    		            title:this.$t('autoHandle').title
	    		        });
				this.$nextTick(() => {
		...
		methods: {
				...
				if (this.palletNo) {
					...
					axios.post(this.apiUrl.apiUrl + '/api/app/queryManager/WmsTask', {
							condition: {
								...
								Type: { in: ["MV_PICKTICKET_PICKING", "MV_WAVE_PICKING","MV_COUNT_OUT"]
								},
								Pallet: {
									'=': this.palletNo
								}
			...
			onSubmit() {
				...
				this.loading = true;
				var _this = this;
				axios.post(this.apiUrl.apiUrl + '/api/app/WorkDocManager/HandleTaskByPallet?pallet='+this.palletNo
					)
					.then(function(response) {
