	 ...
	 <van-search v-model="code" :placeholder="i18n.code[0]"  v-show='i18n.code[1]==1'/>
		<van-list
		  ...
		  :finished-text="i18n.more"
		   ...
		   :offset="130"
		>
		<van-swipe-cell v-for="item in shipOrders">
		  <van-card
		    :title="`${item.xCode}`"
		    class="goods-car"
			@click="OnClick(item.xCode,item.id)"
		  >
		  <template #tags>
		      <van-tag plain type="danger" v-for="item2 in item.shipOrderItem">{{item2.material.xCode+' '+item2.material.xName+' '+(item2.materialProperty.batchNo!=null?item2.materialProperty.batchNo:'')+' '+item2.expectedPkgQuantity+' '+item2.alloactedPkgQuantity+' '+item2.pickedPkgQuantity}}</van-tag>
		    </template>
		</van-card>
...
<script>
	...
	export default {
		...
		methods: {
			...
			axios.post(this.apiUrl.apiUrl + '/api/app/queryManager/shipOrder', input)
