<template>
	<van-form @submit="onSubmit" style="margin:3rem;">
		...
		<van-cell-group v-show='i18n.pickingnumber[1]==1'>
			<van-field v-model="planPkgQuantity" :label="i18n.pickingnumber[0]" type="number" :rules="[{ required: true, message: i18n.planPkgQuantityMsg }]"
			 required>
				<template #button>
					<p>{{quantity}}</p>
				</template>
		...
		<div style="margin: 16px;" v-show='i18n.returnBack[1]==1'>
			<van-button :loading="loading" round block type="info" @click="Rrturn()">
				{{i18n.returnBack[0]}}
			</van-button>
		</div>
...
<script>
	...
	export default {
		...
		computed: {
			i18n() {
				return this.$t('picking')
			}
		},
        mounted:function(){
        		uni.setNavigationBarTitle({
        		            title:this.$t('picking').title
        		        });
				this.$nextTick(() => {
		...
		methods: {
					...
					axios.post(this.apiUrl.apiUrl + '/api/app/queryManager/WmsTask', {
						...
						.then(function(response) {
							...
							if (_this.wmsTasks.length > 0) {
								...
								if(_this.$t('picking').code[2]!=undefined&&this.$t('picking').code[2]!="")
								{
								}else{
						...
						.catch(function(error) { // 请求失败处理
							...
							Toast({
								message: _this.$t('picking').taskErrorMsg
							});
						});
				}
			},
			...
			onSubmit() {
				...
				if(this.wmsTasks.length==0)
				{
					_this.loading = false;
					Toast({
						message: _this.$t('picking').pickingErrorMsg
					});
					
					return;
				}
				
				axios.post(this.apiUrl.apiUrl + '/api/app/WorkDocManager/HandlePickTask', {
					...
					.then(function(response) {
						...
						if (response.data == true) {
							...
							Toast({
								message: _this.$t('picking').pickingSuccessMsg
							});
		...
		Rrturn(){
			this.loading = true;
			var _this = this;
			axios.post(this.apiUrl.apiUrl + '/api/app/WorkDocManager/cancelTaskByPallet?pallet='+this.palletNo)
				.then(function(response) {
					...
					Toast({
						message: _this.$t('picking').retrunSuccessMsg
					});
					}
					...
					.catch(function(error) { // 请求失败处理
						_this.loading = false;
						Toast({
							message: _this.$t('picking').retrunErrorMsg
						});
					});
