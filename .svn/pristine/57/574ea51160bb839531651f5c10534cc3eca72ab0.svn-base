<template>
	<van-form @submit="onSubmit" style="margin:3rem;">
		<van-cell-group v-show='i18n.palletNo[1]==1&&parmater==""'>
			<van-field v-model="palletNo" :label="i18n.palletNo[0]" />
		</van-cell-group>

		<van-cell-group v-show='i18n.fromLocCode[1]==1&&parmater==""'>
		...
		<van-cell-group>
			...
			<van-nav-bar v-bind:title="material" :left-text="i18n.previous[0]" :right-text="i18n.next[0]"
				@click-left="onClickLeft" @click-right="onClickRight" :color="'#4fc08d'" />
		...
		<van-cell-group v-show='i18n.pickingnumber[1]==1'>
			<van-field v-model="planPkgQuantity" :label="i18n.pickingnumber[0]" type="number"
				:rules="[{ required: true, message: i18n.planPkgQuantityMsg }]" required>
				<template #button>
					<p>{{quantity}}</p>
				</template>
			</van-field>
		...
		<van-cell-group v-show='i18n.toFromLoc[1]==1&&parmater!=""'>
			<van-field v-model="toFromLoc" :label="i18n.toFromLoc[0]" readonly />
		</van-cell-group>
		<van-cell-group v-show='i18n.loction[1]==1&&parmater!=""'>
			<van-field v-model="loction" :label="i18n.loction[0]" />
		</van-cell-group>
		...
		methods: {
			onChange() {
				...
				let input = {
					condition: {
						Status: {
							in: ["DISPATCHED", "WORKING"]
						},
						Type: {
							in: ["MV_PICKTICKET_PICKING", "MV_WAVE_PICKING"]
						}
					},
					pageNumber: 1,
					pageSize: 500,
				...
				if (this.palletNo != "") {
					input.condition['Pallet'] = {
						'=': this.palletNo
					}
				}
				if (this.fromLocCode != "") {
					input.condition['fromLocCode'] = {
						'=': this.fromLocCode
					}
				}
				...
				axios.post(this.apiUrl.apiUrl + '/api/app/queryManager/WmsTask', input)
